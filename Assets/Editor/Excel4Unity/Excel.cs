...
public class Excel
{
    ...
    public List<string> GetKeyTables = new List<string>();
    ...
    public Excel(ExcelWorkbook wb)
    {
        ...
        {
            GetKeyTables.Add(wb.Worksheets[i].Name);
    ...
    public Dictionary<string, string> GetSheetDic()
    {
        Dictionary<string, string> SetDic = new Dictionary<string, string>();
        for(int i = 0; i < Tables.Count; i++)
        {
            SetDic.Add(GetKeyTables[i], Tables[i].WhriteSheetLogMyLogic());
        }
